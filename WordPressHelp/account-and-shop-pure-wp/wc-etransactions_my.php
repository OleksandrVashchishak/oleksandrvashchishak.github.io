...
class WC_Etransactions_my {
	...
	public function __construct(WC_Etransactions_Config_my $config) {
		$this->_config = $config;
	}
	 ...
	 */
	public function buildSystemParams($orderId, $form, $price, $type, array $additionalParams = array()) {
		global $wpdb;
		...
		// Order information
		$values['PBX_PORTEUR'] = $form['adresse_email'];
		$values['PBX_DEVISE'] = '978';
		$values['PBX_CMD'] = $orderId.' - '.$form['prenom'].' '.$form['nom'];

		// Amount
		$orderAmount = (int)str_replace(" ", "", $price);
		...
		// Hash HMAC
		update_field( 'key_payment', $sign, $orderId );
