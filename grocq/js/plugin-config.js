...
if (document.querySelector('.tab-slider__slider')) {
    for (let i = 1; i <= 2; i++) {
        $(".tab-slider__slider-" + String(i)).slick({
            slidesToShow: 4,
            infinite: true,
            arrows: true,
            draggable: false,
        ...
        const updateSliderCounter = (slick) => {
            ...
            $(".tab-slider__slider-current-slide-" + String(i)).text(
                Math.ceil(currentSlide)
            );
        };

        $(".tab-slider__slider-" + String(i)).on("init", (slick) => {
            updateSliderCounter(slick);
        });
        let countClonedSlide = 0;
        let countSlide = document.querySelectorAll(
            ".tab-slider__slider-" + String(i) + " .tab-slider__item"
        );
        ...
        let allCountSlide = Math.ceil(
            (countSlide.length - countClonedSlide)
        );
        $(".tab-slider__slider-all-slide-" + String(i)).text(allCountSlide);

        $(".tab-slider__slider-" + String(i)).on(
            "afterChange",
    ...
    // start change slider arrow
    $(".slick-prev").html($(".tab-slider__svg-arrow").html())
    $(".slick-next").html($(".tab-slider__svg-arrow").html())
    // end change slider arrow
    // end tabs slider
}
// end tab slider
...
if (document.querySelector('.header__menu ul')) {

    let sections = $('section')
    let nav = $('.header__menu ul li')
    let nav_height = nav.outerHeight();

    $(window).on('scroll', function () {
        let cur_pos = $(this).scrollTop();
        sections.each(function () {
            let top = $(this).offset().top - nav_height - 80,
    ...
    //     $('html, body').animate({
    //         scrollTop: $(id).offset().top - nav_height - 40
    //     }, 1000);
